...
export class DBSqlite3 implements Database {
	...
	public async connect(config: Config, logger: Logger): Promise<number> {
		...
		try {
			...
			this.logger.critical(
				'Sqlite3Driver',
				Translator.translate('database.connectException') + ': ' + err
			)
			process.exit(1)
		}
		return 0
	...
	public async disconnect(logger: Logger): Promise<number> {
		try {
			...
			logger.critical(
				'Sqlite3Driver',
				Translator.translate('database.disconnectException') + err
			)
			process.exit(1)
			return 1
		}
...
export class DBService implements Service {
	...
	public async onInitialize(
		...
		this.server.getBus().on('stopDB', () => {
			this.db.disconnect(server.getLogger())
		})
	}
