...
export class PermissionService implements Service {
	...
	public async onInitialize(
		...
		try {
			;(
				await this.server.getDB().execute('select * from permissions', true)
			).forEach((dat: any) => {
			...
			this.server
				...
				.critical(
					'Permission',
					Translator.translate('permission.serviceInitializeException')
				)
			process.exit(1)
		}
	}
	...
	public hasPermission(uid: number, permission: number) {
		if (this.permissionCache.has(uid))
	...
	public async setPermission(uid: number, permission: number) {
		if (this.permissionCache.has(uid))
			await this.server
				?.getDB()
				.execute(
					`update permissions set permissions = ${permission} where uid = ${uid}`,
					false
				)
		else {
			await this.server
				?.getDB()
				.execute(
					`insert into permissions (uid, permissions) value (${uid}, ${permission})`,
					false
				)
		}
